...
public class Test18 {
	public static void main(String[] args) {
		...
		String traza = "";
		String mensaje = "El producto se ha registrado correctamente.";
		try {
			// navegar a url
			driver.get(pm.url);
			//Pasos del test
			...
			traza = "Paso 6 => Se pulsa en Registrar Producto";
			admin.pulsarRegistrarProducto();
			mc.writeReport("INFO", "OK", traza, driver, "Paso 6");
			traza = "Paso 7 => Se rellena el formulario con los datos del producto que se quiera registrar.";
			admin.pulsarTipo("MERCHANDISING");
			admin.pulsarTipoMerchan();
			admin.escribirTitulo("Prueba");
			admin.escribirDescripcion("Prueba");
			admin.escribirPrecio("23.99");
			admin.escribirStock("15");
			admin.escribirFecha("06-05-2020");
			admin.escribirFichero();
			mc.writeReport("INFO", "OK", traza, driver, "Paso 7");
			traza = "Paso 8 => Se pulsa en el botón Registrar";
			admin.pulsarRegistrarProd();
			mc.writeReport("INFO", "OK", traza, driver, "Paso 8");
			//comprobación del test
			String res="";
			if (mensaje.equals(admin.leerAlertRegistrar())) {
				res ="OK";
			...
			if (args.length>0) {
				mc.writeExcelReport(test, res, 18, args[0]);
			}
		} catch (Exception e) {
			...
			if (args.length>0) {
				mc.writeExcelReport(test, "KO", 18, args[0]);
			}
			e.printStackTrace();
		} finally {
			//cerrar el driver

...
public class Test14 {
	public static void main(String[] args) {
		...
		String traza = "";
		String mensaje = "Se han concedido permisos de administrador al usuario pruebasfxg@gmail.com";
		try {
			// navegar a url
			driver.get(pm.url);
			//Pasos del test
			...
			traza = "Paso 8 => Se pulsa el botón Dar permisos.";
			admin.pulsarDarPermisos();
			mc.writeReport("INFO", "OK", traza, driver, "Paso 8");
			//comprobación del test
			String res="";
			if (mensaje.equals(admin.leerAlertPermisos())) {
				res ="OK";
			...
			if (args.length>0) {
				mc.writeExcelReport(test, res, 14, args[0]);
			}
		} catch (Exception e) {
			...
			if (args.length>0) {
				mc.writeExcelReport(test, "KO", 14, args[0]);
			}
			e.printStackTrace();
		} finally {
			//cerrar el driver

...
public class Test11 {
	...
	public static void main(String[] args) {
		...
		String traza = "";
		String mensaje = "Su compra se ha realizado con éxito. Gracias.";
		try {
			// navegar a url
			driver.get(pm.url);
			//Pasos del test
			...
			String res="";
			if (mensaje.equals(cesta.leerAlert())) {
				res ="OK";
			...
			if (args.length>0) {
				mc.writeExcelReport(test, res, 11, args[0]);
			}
		} catch (Exception e) {
			...
			if (args.length>0) {
				mc.writeExcelReport(test, "KO", 11, args[0]);
			}
			e.printStackTrace();
		} finally {
			//cerrar el driver

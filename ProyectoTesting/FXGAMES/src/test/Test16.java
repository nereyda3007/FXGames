...
public class Test16 {
	public static void main(String[] args) {
		...
		String traza = "";
		String mensaje = "Las ofertas se han eliminado correctamente.";
		try {
			// navegar a url
			driver.get(pm.url);
			//Pasos del test
			...
			traza = "Paso 6 => Se pulsa en el botón Eliminar ofertas del segundo formulario.";
			admin.pulsarEliminarOfertas();
			mc.writeReport("INFO", "OK", traza, driver, "Paso 6");
			//comprobación del test
			String res="";
			if (mensaje.equals(admin.leerAlertEliminarOfertas())) {
				res ="OK";
			...
			if (args.length>0) {
				mc.writeExcelReport(test, res, 16, args[0]);
			}
		} catch (Exception e) {
			...
			if (args.length>0) {
				mc.writeExcelReport(test, "KO", 16, args[0]);
			}
			e.printStackTrace();
		} finally {
			//cerrar el driver

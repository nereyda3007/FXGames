...
public class Test01 {
	...
	public static void main(String[] args) {
		...
		String traza = "";
		String mensajeAlert = "Su usuario se ha registrado correctamente. Inicie sesión para acceder a su perfil.";
		try {
			// navegar a url
			driver.get(pm.url);
			//Pasos del test
			...
			if (args.length>0) {
				mc.writeExcelReport(test, res, 1, args[0]);
			}
		}catch (Exception e) {
			...
			if (args.length>0) {
				mc.writeExcelReport(test, "KO", 1, args[0]);
			}
			e.printStackTrace();
		} finally {
			//cerrar el driver

...
public class Test19 {
	public static void main(String[] args) {
		...
		String traza = "";
		String mensaje = "La oferta se ha eliminado correctamente.";
		try {
			// navegar a url
			driver.get(pm.url);
			//Pasos del test
			...
			traza = "Paso 6 => Se introduce el nombre del producto a eliminar la oferta, Super Mario Party";
			admin.escribirNProducto("Super Mario Party");
			mc.writeReport("INFO", "OK", traza, driver, "Paso 6");
			traza = "Paso 7 => Se pulsa en el botón Eliminar oferta";
			admin.pulsarEliOFerta();
			mc.writeReport("INFO", "OK", traza, driver, "Paso 7");
			//comprobación del test
			String res="";
			if (mensaje.equals(admin.leerAlertEliOFerta())) {
				res ="OK";
			...
			if (args.length>0) {
				mc.writeExcelReport(test, res, 19, args[0]);
			}
		} catch (Exception e) {
			...
			if (args.length>0) {
				mc.writeExcelReport(test, "KO", 19, args[0]);
			}
			e.printStackTrace();
		} finally {
			//cerrar el driver
